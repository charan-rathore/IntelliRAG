...
This is the separate `rag-platform/` implementation and its historical phase documentation. Mock benchmark results below do not measure the live TypeScript application. Start at [the main README](README.md) for the current product and measured repository-support diagnostic.
...
- Engineers learning AI systems beyond API wrappers
...
The current live lab (hybrid retrieval, corpus isolation, evidence gating, RAGAS + A–N bars) is the Vite console in [`web/`](web/):
...
The Python FastAPI console below is the original `rag-platform` UI.
...
LangChain and LlamaIndex are great for prototypes. This lab exists because:
