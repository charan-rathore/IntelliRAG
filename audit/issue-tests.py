...
for q in questions:
 ...
 print(json.dumps({'question':q,'model':done.get('model'),'answer':done.get('answer'),'coverage':done.get('coverage')}),flush=True)
 (OUT/(SESSION+'-'+('lyra' if len(sys.argv)>2 else 'issue')+'.json')).write_text(json.dumps(results,indent=2))
