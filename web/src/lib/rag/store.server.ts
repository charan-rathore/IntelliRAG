...
export async function replaceChunks(
  ...
  if (!transaction)
    return sql.transaction((tx) => replaceChunks(documentId, body, opts, corpusId, tx));
...
export async function upsertDocument(input: UpsertInput): Promise<{
  ...
  return sql.transaction((tx) => upsertDurable(input, tx));
}

async function upsertDurable(input: UpsertInput, sql: Sql) {
  ...
  // Serialize writers for the same slug, including a document's first insert.
  // This lock is released by COMMIT/ROLLBACK and works across function instances.
  await sql`select pg_advisory_xact_lock(hashtext(${slugBase}))`;
  const byUri = input.sourceUri
  ...
  `;
  const chunkCount = await replaceChunks(id, input.body, chunkOptsFrom(input), corpusId, sql);
