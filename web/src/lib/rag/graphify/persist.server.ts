...
export async function ensureGraph(): Promise<GraphState> {
  ...
  const fingerprint = GRAPH_EXTRACTOR_VERSION + ":answerability-v2:" + JSON.stringify(documents.map(d => [d.slug, d.version]).sort((a, b) => String(a[0]).localeCompare(String(b[0]))));
